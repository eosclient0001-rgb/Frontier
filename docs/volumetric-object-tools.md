...
Water paths move and scale their actual XYZ control points. **Y changes only the selected point or route**, preserving independent river grades. Rendering, current and emission share those elevations plus the optional route surface offset. Width controls the routed footprint; direction controls the GPU current. See [shared routes](satmaps-and-routes.md).
