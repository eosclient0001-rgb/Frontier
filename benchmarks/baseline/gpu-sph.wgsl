...
struct Params {
  ...
  poke: vec4<f32>, grid: vec4<u32>, settings: vec4<f32>, rock: vec4<f32>
}
...
fn densities(@builtin(global_invocation_id) id:vec3<u32>){
  ...
  let poly=315./(64.*PI*pow(h,9.));var rho=0.;
  for(var z=-1;z<=1;z++){for(var y=-1;y<=1;y++){for(var x=-1;x<=1;x++){
    ...
    loop {
      ...
      let d=p-src[j].p.xyz;let q=max(0.,h2-dot(d,d));rho+=u.fluid.x*poly*q*q*q;j=links[j];
    }
  }}}
...
fn integrate(@builtin(global_invocation_id) id:vec3<u32>){
  ...
  for(var z=-1;z<=1;z++){for(var y=-1;y<=1;y++){for(var x=-1;x<=1;x++){
    ...
    var j=atomicLoad(&heads[key(other)]);var visits=0u;
    loop {
      if(j==NIL||visits>=256u){break;}visits++;
      if(i!=j){
        let delta=p-src[j].p.xyz;let r=length(delta);
        if(r<h&&r>.00001){
          let q=h-r;let rj=density[j].x;let pj=density[j].y;
          let kernel=h*h-r*r;smoothing+=2.*u.fluid.x/(rho+rj)*poly*kernel*kernel*kernel*(src[j].v.xyz-v);
          acceleration+=u.fluid.x*(pressure/(rho*rho)+pj/(rj*rj))*spiky*q*q*delta/r;
          acceleration+=u.fluid.w*u.fluid.x/rj*spiky*q*(src[j].v.xyz-v);
          acceleration-=u.settings.z*(q/h)*(q/h)*delta/r;
        }
      }
      j=links[j];
    }
  ...
  v+=acceleration*u.motion.w+smoothing*(.03*u.motion.w/.002);
