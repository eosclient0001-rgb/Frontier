...
fn box(origin:vec3<f32>,direction:vec3<f32>,center:vec3<f32>,halfSize:vec3<f32>,color:vec3<f32>,previous:Hit)->Hit{
  let inv=1./(direction+vec3<f32>(.0000001));let a=(center-halfSize-origin)*inv;let b=(center+halfSize-origin)*inv;
  let mn=min(a,b);let mx=max(a,b);let near=max(max(mn.x,mn.y),mn.z);let far=min(min(mx.x,mx.y),mx.z);
  if(near<=0.||near>far||near>=previous.distance){return previous;}
  let point=(origin+direction*near-center)/halfSize;let ap=abs(point);var normal=vec3<f32>(0.,0.,sign(point.z));
  if(ap.x>ap.y&&ap.x>ap.z){normal=vec3<f32>(sign(point.x),0.,0.);}else if(ap.y>ap.z){normal=vec3<f32>(0.,sign(point.y),0.);}
  return Hit(near,normal,color);
}
fn scene(origin:vec3<f32>,direction:vec3<f32>)->Hit{
  var hit=Hit(1000.,vec3<f32>(0.,1.,0.),vec3<f32>(.014,.026,.036));
  ...
  let wall=vec3<f32>(.12,.19,.23);
  hit=box(origin,direction,vec3<f32>(cam.domain.x+.09,.64,0.),vec3<f32>(.09,.64,cam.domain.z+.18),wall,hit);
  hit=box(origin,direction,vec3<f32>(-cam.domain.x-.09,.64,0.),vec3<f32>(.09,.64,cam.domain.z+.18),wall,hit);
  hit=box(origin,direction,vec3<f32>(0.,.64,cam.domain.z+.09),vec3<f32>(cam.domain.x,.64,.09),wall,hit);
  hit=box(origin,direction,vec3<f32>(0.,.64,-cam.domain.z-.09),vec3<f32>(cam.domain.x,.64,.09),wall,hit);
...
@fragment fn fragment(@builtin(position) position:vec4<f32>)->@location(0) vec4<f32>{
  ...
  let background=scene(origin,direction);var color=lighting(background);
  let z=textureLoad(depth,xy,0).r;
  if(z>0.){
    let vp=viewPoint(xy,z);let wp=(cam.world*vec4<f32>(vp,1.)).xyz;
    if(length(wp-origin)<background.distance){
