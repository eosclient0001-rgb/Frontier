export const PRESETS={6144:[32,8,24],12000:[40,10,30],24000:[50,12,40]};
export class GPUSPH {
  static async create(device,count=6144,dimensions=[5,3.4]){
    const solver=new GPUSPH(device,count,dimensions);await solver.init();return solver;
  }
  constructor(device,count,dimensions=[5,3.4]){
    const [width,depth]=dimensions;if(!Number.isFinite(width)||!Number.isFinite(depth)||width<5||width>14||depth<3.4||depth>10)throw new Error('Tub dimensions must be 5–14 m wide and 3.4–10 m long.');
    this.device=device;this.count=count;this.spacing=.12*Math.cbrt(6144/count*width*depth/17);this.h=2*this.spacing;
    this.radius=.40*this.spacing;this.mass=1000*this.spacing**3;this.dt=Math.min(.002,this.h/120);
    this.domain=[width/2,depth/2,3.0];this.grid=[Math.ceil(width/this.h),Math.ceil(3/this.h),Math.ceil(depth/this.h)];
    this.cells=this.grid.reduce((a,b)=>a*b,1);this.rock=[-1.05,.32,-.55,.48];this.body=[.9,.68,.25,.40];
    this.time=0;this.steps=0;this.current=0;this.maxSteps=12;this.viscosity=.025;
  }
  async init(){
    const d=this.device;const make=(size,usage,label)=>d.createBuffer({size:Math.ceil(size/4)*4,usage,label});
    ...
    const response=await fetch(new URL('./gpu-sph.wgsl?v=editor2',import.meta.url));if(!response.ok)throw new Error('Could not load GPU compute shader: '+response.status);
    const shader=d.createShaderModule({code:await response.text(),label:'3D WCSPH compute'});
    ...
    const entries=[{binding:0,visibility:GPUShaderStage.COMPUTE,buffer:{type:'uniform',hasDynamicOffset:true,minBindingSize:128}}];
    ...
    for(const entryPoint of ['clear','build','densities','integrate'])this.pipelines[entryPoint]=await d.createComputePipelineAsync({label:entryPoint,layout:pipelineLayout,compute:{module:shader,entryPoint}});
    this.groups=[0,1].map(i=>d.createBindGroup({layout:this.layout,entries:[{binding:0,resource:{buffer:this.params,offset:0,size:128}},...[[1,this.buffers[i]],[2,this.buffers[1-i]],[3,this.heads],[4,this.links],[5,this.density],[6,this.counters]].map(([binding,buffer])=>({binding,resource:{buffer}}))]}));
    this.reset();
  }
  reset(){
    const original=this.domain[0]===2.5&&this.domain[1]===1.7;
    const nx=original?PRESETS[this.count][0]:Math.max(2,Math.floor(this.domain[0]*1.6/this.spacing));
    const nz=original?PRESETS[this.count][2]:Math.max(2,Math.floor(this.domain[1]*1.68/this.spacing));const data=new Float32Array(this.count*8);let index=0;
  ...
  encode(encoder,steps,target,poke,timestamps){
    ...
    for(let step=0;step<steps;step++){
      ...
      data.set([this.mass,1000,100,this.viscosity],offset+4);data.set(body,offset+8);
      data.set([...velocity,this.dt],offset+12);data.set(poke,offset+16);
      ints.set([...this.grid,this.count],offset+20);data.set([this.radius,9.81,.06,.08],offset+24);data.set(this.rock,offset+28);
    }
    ...
    for(let step=0;step<steps;step++){
      ...
      for(const name of ['clear','build','densities','integrate']){
  ...
  async inspect(){
    ...
    const d=this.device,size=this.count*32+16;const staging=d.createBuffer({size,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ});
    const encoder=d.createCommandEncoder();encoder.copyBufferToBuffer(this.buffers[this.current],0,staging,0,this.count*32);encoder.copyBufferToBuffer(this.counters,0,staging,this.count*32,16);d.queue.submit([encoder.finish()]);
    ...
    let finite=true,minY=Infinity,maxY=-Infinity,maxSpeed=0,insideSolids=0,outside=0,meanDensity=0;
    for(let i=0;i<this.count;i++){
      const k=i*8,x=p[k],y=p[k+1],z=p[k+2];for(let j=0;j<8;j++)finite&&=Number.isFinite(p[k+j]);
      minY=Math.min(minY,y);maxY=Math.max(maxY,y);maxSpeed=Math.max(maxSpeed,Math.hypot(p[k+4],p[k+5],p[k+6]));meanDensity+=p[k+3];
      ...
      if(Math.hypot(x-this.rock[0],y-this.rock[1],z-this.rock[2])<this.rock[3]+this.radius-.002||Math.hypot(x-this.body[0],y-this.body[1],z-this.body[2])<this.body[3]+this.radius-.002||Math.hypot(x-this.body[0],y-this.body[1]-.34,z-this.body[2]-.17)<.24+this.radius-.002)insideSolids++;
    }
    staging.unmap();staging.destroy();return {count:this.count,time:this.time,steps:this.steps,finite,minY,maxY,maxSpeed,meanDensityRatio:meanDensity/this.count,outside,insideSolids,neighborTruncations:counters[0],speedCaps:counters[1],contacts:counters[2],invalidResets:counters[3]};
  }
  destroy(){for(const b of [...this.buffers,this.heads,this.links,this.density,this.counters,this.params])b.destroy();}
}
