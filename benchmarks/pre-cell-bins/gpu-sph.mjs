...
export class GPUSPH {
  ...
  async init(){
    ...
    const response=await fetch(new URL('./gpu-sph.wgsl?v=opt5',import.meta.url));if(!response.ok)throw new Error('Could not load GPU compute shader: '+response.status);
    ...
    for(let i=1;i<=7;i++)entries.push({binding:i,visibility:GPUShaderStage.COMPUTE,buffer:{type:i===1?'read-only-storage':'storage'}});
    ...
    this.groups=[0,1].map(i=>d.createBindGroup({layout:this.layout,entries:[{binding:0,resource:{buffer:this.params,offset:0,size:144}},...[[1,this.buffers[i]],[2,this.buffers[1-i]],[3,this.heads],[4,this.links],[5,this.density],[6,this.counters],[7,this.neighbors]].map(([binding,buffer])=>({binding,resource:{buffer}}))]}));
    this.reset();
  }
  reset(){
  ...
  async inspect(){
    ...
    staging.unmap();staging.destroy();return {densityError,count:this.count,time:this.time,steps:this.steps,finite,minY,maxY,maxSpeed,meanDensityRatio:meanDensity/this.count,kineticEnergy,outside,insideSolids,penetrations,neighborTruncations:counters[0],speedCaps:counters[1],contacts:counters[2],invalidResets:counters[3]};
  }
  destroy(){for(const b of [...this.buffers,this.heads,this.links,this.density,this.counters,this.params,this.neighbors])b.destroy();}
}
