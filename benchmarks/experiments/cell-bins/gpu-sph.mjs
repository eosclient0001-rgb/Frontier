...
export class GPUSPH {
  ...
  constructor(device,count,dimensions=[5,3.4]){
    ...
    this.time=0;this.steps=0;this.current=0;this.maxSteps=12;this.viscosity=.003;this.velocitySmoothing=.004;this.cohesion=0;this.gravity=9.81;this.stiffness=100;
  }
  async init(){
    ...
    this.bins=make(this.cells*33*4,GPUBufferUsage.STORAGE,'Compact cell bins with overflow fallback');
    ...
    const response=await fetch(new URL('./gpu-sph.wgsl?v=opt4',import.meta.url));if(!response.ok)throw new Error('Could not load GPU compute shader: '+response.status);
    ...
    this.groups=[0,1].map(i=>d.createBindGroup({layout:this.layout,entries:[{binding:0,resource:{buffer:this.params,offset:0,size:144}},...[[1,this.buffers[i]],[2,this.buffers[1-i]],[3,this.heads],[4,this.links],[5,this.density],[6,this.counters],[7,this.neighbors],[8,this.bins]].map(([binding,buffer])=>({binding,resource:{buffer}}))]}));
    this.reset();
  }
  reset(){
  ...
  encode(encoder,steps,target,poke,timestamps){
    ...
    for(let step=0;step<steps;step++){
      ...
      data.set([this.mass,1000,this.stiffness,this.viscosity],offset+4);data.set(body,offset+8);
      data.set([...velocity,this.dt],offset+12);data.set(poke,offset+16);
      ints.set([...this.grid,this.count],offset+20);data.set([this.radius,this.gravity,this.cohesion,.08],offset+24);data.set(this.rock,offset+28);data.set([this.velocitySmoothing,0,0,0],offset+32);
    }
  ...
  async inspect(){
    ...
    let finite=true,minY=Infinity,maxY=-Infinity,maxSpeed=0,insideSolids=0,outside=0,meanDensity=0,kineticEnergy=0,penetrations=[];
    ...
    staging.unmap();staging.destroy();return {count:this.count,time:this.time,steps:this.steps,finite,minY,maxY,maxSpeed,meanDensityRatio:meanDensity/this.count,kineticEnergy,outside,insideSolids,penetrations,neighborTruncations:counters[0],speedCaps:counters[1],contacts:counters[2],invalidResets:counters[3]};
  }
  destroy(){for(const b of [...this.buffers,this.heads,this.links,this.density,this.counters,this.params,this.neighbors,this.bins])b.destroy();}
}
