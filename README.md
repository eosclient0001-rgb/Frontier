...
| F | bite (cycles three distinct orthal bite variants) |
...
when it gets close:
* **Bite:** the ball is in front, roughly 3.25–7 m from the hips. The animal braces rather than folding at the waist: the tooth row is aimed down, the lower jaw opens and closes vertically, then the closed skull makes a short puncture-and-pull before recovery. Repeated bites cycle a restrained puncture-pull, a small lateral rake, and a lift-and-crush variant. On a catch it gives only a restrained shake and flings the ball.
...
The skull carries two digital alien receiver antennae mounted just behind the eye sockets; their segmented stalks rake upward and backward toward the posterior skull. Two hydraulic jaw actuators are anchored outside the mouth on the lateral cheek and ramus, so their barrels and pistons visibly compress and extend with the live jaw transform. Exactly three electrical cables run from the throat bus down and posteriorly to the ventral stomach/torso: one emissive yellow main cable and orange and red secondary cables. They use Verlet integration with segment constraints, gravity, damping and moving anatomical anchors, so they sag and lag during turns and attacks instead of staying as rigid lines. The pelvis/acetabula and IK hip joints are offset left and right from the midline; turns articulate the pelvis, trunk vertebrae, neck, head and trailing tail with delayed counter-rotation.

During a hunt the brain runs when the ball is far away, transitions to a controlled walk in the close envelope, and resumes a sensible approach when the ball moves away. Dragging or throwing the ball is treated as a fresh target rather than leaving the animal in a stale low-speed reposition state.
