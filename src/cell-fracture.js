import { fractureScale } from "./domain.js";
...
export function generateCellPattern(
  ...
  if (
    ...
    size < 2 * fractureScale() ||
    size > 12 * fractureScale() ||
    depth < fractureScale() ||
    depth > 50 * fractureScale() ||
    variation < 0 ||
    variation > 1
  )
  ...
  return {
    ...
    gap: detail
      ...
      : Math.max(0.8 * fractureScale(), Math.min(2 * fractureScale(), gap)),
    separate: !!separate,
    detail: !!detail,
    intact: !!intact,
    requestedSize: size,
