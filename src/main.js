import { fineCapacityText } from "./fine-capacity-status.js";
import { rainParameters } from "./rain-physics.js";
import { erosionResolutionText } from "./erosion-resolution.js";
import { DEFAULT_WORLD_PARTICLES, DEFAULT_HYDRAULIC_PARTICLES, MAX_HYDRAULIC_PARTICLES, MAX_VISIBLE_CARRIERS, RAIN_PARTICLES } from "./particle-pool.js";
import {
  performancePresets,
  erosionTiming,
  acceleratedSolidChange,
} from "./erosion-performance.js";
import { worldDefaults } from "./world.js";
import {
  configureDomain,
  CELL,
  MIN,
...
import { configureTerrainTools } from "./terrain-tools.js";
import { terrainDefaults, terrainNames } from "./noise-terrain.js";
import { createNoiseTerrainUI } from "./noise-terrain-ui.js";
import "./studio-ui.js";
...
import { defaults, SIZE, encodeScene, sampleVolume } from "./field.js";
...
const params = {
  ...defaults,
  ...worldDefaults,
  ...performancePresets.fast,
  particleCount: DEFAULT_WORLD_PARTICLES,
  hydraulicShaping: true,
  hydraulicCell: 0.5,
  hydraulicSamples: DEFAULT_HYDRAULIC_PARTICLES,
  hydraulicWater: 8,
  speed: 2,
  preset: 4,
  waterEnabled: false,
...
let fractureTool, sceneEditor, terrainGenerator;
let renderer,
  canvas = $("#scene"),
  lastTime = 0,
  lastSim = 0,
...
const worker = new FieldWorker(undefined, {
  onProgress: ({ type, progress }) => {
    if (state.ready) return;
    $("#loading strong").textContent = "Building the whole-terrain XYZ volume";
    $("#loading>span").textContent =
      `${Math.round(progress * 100)}% · Terrain worker is running`;
  },
});
const norm = (v) => {
...
function updateSimulation() {
  ...
  $("#sim-detail").textContent = state.running
    ? `${params.hydraulicShaping ? "Hydraulic shaping" : profileFor(params.sourceMode).name} · GPU transport`
    : state.iterations
      ? "Simulation paused"
...
function updatePerformance() {
  const capacityNote = $("#hydraulic-capacity-status");
  if (capacityNote) {
    const report = renderer?.solver?.fine?.capacityStatus;
    capacityNote.textContent = fineCapacityText(report);
    capacityNote.classList.toggle("capacity-warning", !!report?.missing);
  }
  const hydraulicCountNote = $("#hydraulic-particle-note");
  if (hydraulicCountNote) hydraulicCountNote.textContent =
    `${params.hydraulicSamples.toLocaleString()} simulated particle slots · up to ${MAX_HYDRAULIC_PARTICLES.toLocaleString()}. More particles supply more authoring water at the same water-per-parcel setting, not larger cuts. Births fill the selected pool over several updates.${params.hydraulicSamples > MAX_VISIBLE_CARRIERS ? ` Only a sampled subset of up to ${MAX_VISIBLE_CARRIERS.toLocaleString()} markers is drawn; all selected slots are simulated.` : ""} High counts cost more GPU time and can exhaust the 4,096-brick fine-detail budget sooner; unallocated contacts cannot erode. Use Audit to check active particles and missing contacts.`;

  $("#app").classList.toggle("hydraulic-shaping", !!params.hydraulicShaping);
  const workflowNote = $("#erosion-workflow-note");
  if (workflowNote)
    workflowNote.textContent = params.hydraulicShaping
      ? "Hydraulic shaping uses bounded iterations, not a timed rainstorm. Fine-grid transport limits travel per iteration. Fast/Rapid process 2/4 iterations per update without widening the erosion kernel."
      : "Fast/Rapid advance transport time without multiplying rainfall or erodibility. Artistic erodibility is not physical geological time.";
  const resolutionNote = $("#erosion-resolution-status");
  if (resolutionNote)
    resolutionNote.textContent =
      erosionResolutionText(params) +
      (renderer?.refinementError ? ` ${renderer.refinementError}` : "");
  const timing = erosionTiming(params);
  if (params.hydraulicShaping) {
    timing.dt = Math.min(timing.dt, params.hydraulicCell / 4);
    timing.effective = timing.dt / 0.04;
  }
  for (const b of $$("[data-erosion-preset]")) {
    const p = performancePresets[b.dataset.erosionPreset],
      on =
        params.timeLapse === p.timeLapse &&
        params.weatheringRate === p.weatheringRate;
    b.classList.toggle("active", on);
    b.setAttribute("aria-pressed", String(on));
  }
  const el = $("#erosion-performance-status");
  if (!el) return;
  const seconds = renderer?.solver?.simulatedSeconds ?? 0;
  const cap = acceleratedSolidChange(params) * 2 * currentDomain().band * 1000;
  const rainRate = rainParameters(params)[0] * params.rainfall;
  if (params.hydraulicShaping) {
    el.textContent = `Hydraulic shaping · ${params.speed} iterations/update · ${params.hydraulicSamples} water parcels · ${params.hydraulicCell} m XYZ cells · ${(params.hydraulicCell * 0.04 * 1000).toFixed(0)} mm maximum change/pass. Iterative terrain authoring, not a timed rainstorm.`;
    return;
  }
  el.textContent = `${timing.effective.toFixed(1)}× transport · ${timing.exposure}× ${timing.exposure > 1 ? "ARTISTIC erodibility (not physical time)" : "erodibility"} · ${params.particleCount} carriers${params.sourceMode === 0 ? ` · ${rainRate.toFixed(1)} mm/h requested rain` : ""} · ${seconds.toFixed(1)} simulated s. Numerical ceiling ${cap.toFixed(2)} mm/pass.${params.sourceMode === 0 && params.particleCount < 4096 ? " Low carrier counts can limit rainfall sampling; the water audit reports unrepresented rain." : ""}`;
}
$$("[data-erosion-preset]").forEach(
  (b) =>
    (b.onclick = () => {
      Object.assign(params, performancePresets[b.dataset.erosionPreset]);
      if (params.hydraulicShaping)
        params.speed =
          b.dataset.erosionPreset === "reference"
            ? 1
            : b.dataset.erosionPreset === "fast"
              ? 2
              : 4;
      syncControls();
      updateSimulation();
      toast(
        "Erosion speed updated without resetting terrain or adding GPU particles.",
      );
    }),
);
function formatValue(id, v) {
  if (id === "hydraulicSamples") return Number(v).toLocaleString();
  if (id === "hydraulicWater") return `${v} m³ / parcel`;
  if (id === "runoffLifetime") return `${v} s`;
  if (id === "runoffEvaporation") return `${(v * 100).toFixed(2)}% / s`;
  if (id === "runoffHead") return `${v.toFixed(2)} m`;
  if (["timeLapse", "weatheringRate"].includes(id)) return `${v}×`;
  if (id === "waterOffset") return `${Number(v).toFixed(2)} m`;
  if (id.startsWith("satmap"))
    return id === "satmapScale"
      ? `${Number(v).toFixed(2)}×`
      : `${Math.round(v * 100)}%`;
  if (id === "rainRate") return `${v.toFixed(0)} mm/h`;
  if (id === "rainSubstrateShear") return `${v.toFixed(0)} Pa`;
  if (id === "rainSubstrateErodibility") return `${v.toFixed(4)}×`;
  if (id === "hydraulicDepth") return `${v} mm`;
  if (id === "hydraulicMaxChange") return `${v} mm/s`;
  if (id === "hydraulicPreview") return `${v}×`;
  if (["hydraulicDrag", "hydraulicErodibility"].includes(id))
    return Number(v).toFixed(3);
  if (["rainLifetime", "rainStallTime"].includes(id))
    return `${Number(v).toFixed(1)} s`;
  if (id === "rainEvaporation") return `${Number((v * 100).toFixed(2))}% / s`;
  if (id === "particleCount") return String(v);
  if (["waveLength", "foamReach", "foamScale"].includes(id))
    return `${Number(v).toFixed(1)} m`;
  if (id === "waveHeight") return `${Math.round(v * 100)} cm`;
  if (["waveSpeed"].includes(id)) return `${Number(v).toFixed(2)}×`;
  ...
  if (["relief", "radius", "waterLevel"].includes(id))
    return `${Number(v).toFixed(1)} m`;
  if (id === "speed") return `${v} passes`;
  if (id === "sun") return `${v}°`;
  return Number(v).toFixed(2);
}
function syncControls() {
  $("#erosion-workflow").value = params.hydraulicShaping
    ? "hydraulic"
    : "weather";
  $("#hydraulic-cell").value = String(params.hydraulicCell);
  $("#hydraulic-source").value = params.sourceMode === 2 ? "2" : "0";
  $("#weather-description").textContent = profileFor(
    params.sourceMode,
  ).description;
  updatePerformance();
  $("#satmap-view").value = params.satmapView;
  $("#satmap-toggle").classList.toggle("on", params.satmapEnabled);
  $("#satmap-toggle").setAttribute(
    "aria-checked",
    String(params.satmapEnabled),
  );
  $$("[data-satmap-palette]").forEach((b) => {
    const on = Number(b.dataset.satmapPalette) === params.satmapPalette;
    b.classList.toggle("active", on);
    b.setAttribute("aria-pressed", String(on));
  });
  for (const input of $$(
    "input[type=range]:not([data-fracture]):not([data-editor]):not([data-terrain])",
  )) {
  ...
  $("#noise-terrain-controls").classList.toggle("hidden", params.preset !== 4);
  $("#legacy-base-warning").classList.toggle("hidden", params.preset === 4);
  for (const [id, key] of [
    ["water-toggle", "waterEnabled"],
    ["river-toggle", "riverEnabled"],
  ]) {
    $("#" + id).classList.toggle("on", params[key]);
    $("#" + id).setAttribute("aria-checked", String(params[key]));
  }
  ...
  try {
    ...
    do {
      ...
      if (revision === state.revision) {
        renderer.setDomain(params);
        configureTerrainTools(params, true);
        renderer.upload(result.volume);
      }
...
function choosePreset(i) {
  if (!state.ready) return;
  if (state.busy || state.rebuilding || state.regenPending) return;
  const p = presets[i];
  if (!p) return;
  if (i === 4) Object.assign(params, terrainDefaults, worldDefaults);
  params.worldEnabled = i === 4;
  params.waterEnabled = false;
  params.riverEnabled = false;
  params.sceneObjects = [];
...
$("#erosion-workflow").onchange = () => {
  const shaping = $("#erosion-workflow").value === "hydraulic";
  if (
    !confirm(
      "Switching erosion workflow regenerates the terrain and clears the current result. Export first if you want to keep it. Continue?",
    )
  ) {
    $("#erosion-workflow").value = params.hydraulicShaping
      ? "hydraulic"
  ...
  setRun(false);
  params.hydraulicShaping = shaping;
  params.speed = shaping
    ? params.timeLapse === 1
      ? 1
      : params.timeLapse <= 4
        ? 2
...
$("#hydraulic-source").onchange = () => {
  setRun(false);
  $("[data-source='" + $("#hydraulic-source").value + "']").click();
};
$("#hydraulic-cell").onchange = () => {
  const value = Number($("#hydraulic-cell").value);
  if (
    state.iterations > 0 &&
    !confirm(
      "Changing cell spacing regenerates the terrain and clears erosion. Export first if you want to keep this result. Continue?",
    )
  ) {
    $("#hydraulic-cell").value = String(params.hydraulicCell);
    return;
  ...
  setRun(false);
  params.hydraulicCell = value;
  rebuild();
};
$("#inspect-hydraulic-detail").onclick = async () => {
  if (!renderer?.gpuErosion) return;
  setRun(false);
  await operation;
  state.busy = true;
  try {
    const changed = renderer.solver.fine?.focusPoint(),
      point =
        changed ??
        (await renderer.solver.pick([0, MAX[1] - 0.1, 0], [0, -1, 0]));
    if (point) {
      camera.target = point;
      camera.distance = changed ? Math.max(12, params.hydraulicCell * 40) : 100;
      camera.pitch = 0.65;
      state.dirty = true;
    }
  } finally {
...
$$("[data-source]").forEach(
  (button) =>
    (button.onclick = () => {
      ...
      params.footprint = Math.max(minFoot, Math.min(maxFoot, params.footprint));
...
$("#audit-erosion").onclick = async () => {
  ...
  output.textContent = "Reading GPU water and sediment ledgers…";
  try {
    await operation;
    state.busy = true;
    const a = await renderer.auditErosion();
    output.textContent = `Selected slots ${a.selectedParticles}\nActive agents  ${a.totalActive}\nRain airborne  ${a.precipitationAirborne}\nSettling       ${a.settling}\nOldest agent   ${a.maxAge.toFixed(2)} s\nDetached       ${a.eroded.toPrecision(6)} m³\nDeposited      ${a.deposited.toPrecision(6)} m³\nIn transport   ${a.carried.toPrecision(6)} m³\nSand           ${a.composition.sand.toFixed(4)} m³\nWet fines      ${a.composition.fines.toFixed(4)} m³\nCoarse chips   ${a.composition.coarse.toFixed(4)} m³\nDissolved      ${a.composition.dissolved.toFixed(4)} m³\nRetired/outflow ${a.retired.toFixed(4)} m³\nLedger error   ${a.ledgerError.toExponential(2)} m³\nVolume error   ${a.massError === null ? "N/A after sculpting" : a.massError.toExponential(2) + " m³"}\nVoxel-volume accounting; not physical calibration.`;
    if (a.refinement)
      output.textContent += `\n\nFINE XYZ FIELD\nCell spacing   ${a.refinement.cellMetres} m\nBricks         ${a.refinement.bricks} / ${a.refinement.capacity}\nMissing contacts ${a.refinement.missingContacts}\nFine removed   ${a.refinement.removed.toPrecision(6)} m³`;
    const w = a.rainWater;
    output.textContent += `\n\nSOURCE WATER — m³\nRequested      ${w.requested.toFixed(6)}\nAdmitted       ${w.admitted.toFixed(6)}\nUnrepresented  ${w.unrepresented.toFixed(6)}\nStored         ${w.stored.toFixed(6)}\nEvaporated     ${w.evaporated.toFixed(6)}\nUnresolved     ${w.unresolvedOutflow.toFixed(6)}\nBalance error  ${w.balanceError.toExponential(2)}\nUnrepresented rain is not secretly added later.`;
  } catch (error) {
...
async function step(count = 1) {
  ...
  try {
    ...
    renderer.solver?.fine?.sampleCapacityStatus();
    state.dirty = true;
    updateSimulation();
  } catch (error) {
    fatal(error.message);
...
$("#satmap-toggle").onclick = () => {
  params.satmapEnabled = !params.satmapEnabled;
  state.dirty = true;
  syncControls();
};
$("#satmap-view").onchange = () => {
  params.satmapView = Number($("#satmap-view").value);
  state.dirty = true;
};
$$("[data-satmap-palette]").forEach(
  (b) =>
    (b.onclick = () => {
      params.satmapPalette = Number(b.dataset.satmapPalette);
      params.satmapEnabled = true;
      state.dirty = true;
      syncControls();
    }),
);
$$("[data-draw-route]").forEach(
  (b) =>
    (b.onclick = () => {
      setRun(false);
      $("[data-source='2']").click();
      sceneEditor?.drawWater();
      syncControls();
    }),
);
...
function resetCamera() {
  ...
  camera.distance =
    (params.worldEnabled
      ? Math.max(params.terrainWidth, params.terrainLength) * 1.6
      : 46) *
    Math.min(
      1.6,
...
$("#export").onclick = async () => {
  ...
  try {
    ...
    const blob = encodeScene(
      ...
      { ...camera },
      state.iterations,
      renderer.solver?.fine?.snapshot() ?? null,
    );
    ...
    toast("Whole terrain exported · XYZ SDF, world bounds and scene settings.");
  } catch (error) {
  ...
  canvas.addEventListener("pointermove", (e) => {
    ...
    if (touches.size === 2) {
      ...
      camera.distance = Math.max(
        ...
        Math.min(
          params.worldEnabled
            ? Math.max(params.terrainWidth, params.terrainLength) * 4
            : 90,
          (camera.distance * lastTouchDistance) / Math.max(1, d),
        ),
      );
      lastTouchDistance = d;
      return;
    ...
    (e) => {
      ...
      if ((navigation.mode === "fly" || navigation.looking) && !e.altKey) {
        params.cameraSpeed = Math.max(
          0.5,
          Math.min(400, params.cameraSpeed * Math.exp(-e.deltaY * 0.001)),
        );
        syncControls();
        return;
      }
...
function frame(now) {
  ...
  if (!state.ready || document.hidden || drawPending || state.busy) return;
  ...
  try {
    ...
    const completion = renderer.draw(uniforms, true);
    if (completion) {
      drawPending = true;
      completion
        .then(() => {
...
async function init() {
  ...
  try {
    ...
    $("#voxel-label").textContent = "1.31M WORLD VOXELS";
    ...
    window.frontier = {
      ...
      get world() {
        return { mode: "whole-volume", dimensions: SIZE, ...currentDomain() };
      },
      // Dense base only; use readTerrain for the exact refined representation.
      readVolume: () => renderer.readVolume(),
      readTerrain: async () => ({
        base: await renderer.readVolume(),
        refinement: renderer.solver?.fine?.snapshot() ?? null,
      }),
      auditErosion: () => renderer.auditErosion(),
    };
  } catch (error) {
    state.startupDiagnostics = error.graphicsDiagnostics ?? null;
    console.error(error);
    fatal(error.message);
    $("#renderer-label").textContent = "Graphics unavailable";
  }
}
terrainGenerator = createNoiseTerrainUI({
  params,
  canEdit: () =>
    state.ready &&
    !state.busy &&
  ...
  generate: async (draft) => {
    setRun(false);
    Object.assign(params, draft, { seed: draft.terrainSeed });
    $("#biome-name").textContent = terrainNames[params.terrainForm];
    resetCamera();
    await rebuild();
    if (!state.ready)
      throw new Error("Terrain generation failed. See renderer diagnostics.");
  },
  erode: () => activateTab("erosion"),
});
document.getElementById("world-overview").onclick = resetCamera;
init();
