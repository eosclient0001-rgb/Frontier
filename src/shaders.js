...
vec3 rockColor(vec3 p,vec3 n){
 float grain=noise(p*8.),broadNoise=noise(p*.43)+.45*noise(p*1.8);
 float microNoise=noise(p*22.)*.5+noise(p*48.)*.25; // crisp sub-voxel micro-detail
 float bedding=p.y+noise(vec3(p.x*.12,0,p.z*.12))*.4;
 float bands=.5+.5*sin(bedding*3.4+.4*sin(bedding*1.1));
 float thin=pow(.5+.5*sin(bedding*17.+noise(p*2.)*1.7),12.);
 // Razor-sharp geometric strata steps and high-frequency erosion rills
 float rillNetwork=pow(1.-abs(noise(vec3(p.x*4.5,p.y*0.8,p.z*4.5))*2.-1.),6.);
 vec3 c=mix(vec3(.37,.135,.064),vec3(.72,.33,.14),.37+broadNoise*.31);
 c=mix(c,c*vec3(.74,.70,.65),bands*surface.x*.35);
 c*=1.-thin*.22*surface.x;
 c+=vec3(.065,.048,.032)*(grain+microNoise-0.75)*surface.y;
 float top=smoothstep(.55,.96,n.y);
 c=mix(c,vec3(.64,.37,.185)*(.9+broadNoise*.12),top*.67);
 float streak=noise(vec3(p.x*3.3,p.y*.13,p.z*3.3));
 c*=.79+.28*streak;
 c*=1.-rillNetwork*.28*surface.y; // etch crisp dendritic tributary dark lines
 float fractures=pow(1.-abs(noise(p*vec3(2.8,.24,2.8))*2.-1.),18.);
 c*=1.-fractures*.22*surface.y;
 if(extra.x>.5)c=vec3(.58,.55,.47);
 return c;
}
