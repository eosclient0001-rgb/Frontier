...
import { EDIT_MIN, EDIT_MAX, CELL } from "./domain.js";
...
export function createObjectTools({
  ...
  for (const [k, axis] of [..."xyz"].entries())
    $("#transform-" + axis).onchange = () => {
      ...
      const p = pivot();
      p[k] = clamp(value, EDIT_MIN[k], EDIT_MAX[k]);
  ...
  function refresh() {
    const n = current(),
      show =
        !!n &&
        !["sculpt", "erosion", "water", "fracture", "satmaps"].includes(
    ...
    $("#transform-help").textContent = n.baked
      ...
      : n.type === "plot"
        ...
        : n.type === "water"
          ? "G Move / R Rotate / S Scale. Rotation is around Y; point elevations preserve the river grade. Y translation moves only the selected point or route, not other rivers."
          : n.type === "shape"
  ...
  function axes(rect) {
    ...
    const eye = cameraEye(camera),
      forward = cameraBasis(camera).forward;
    const depth = p.reduce((sum, v, k) => sum + (v - eye[k]) * forward[k], 0);
    // Keep handles usable on screen at kilometre scale and during close inspection.
    const length = Math.max(
      4,
      Math.max(...CELL) * 2,
      (depth * 1.24 * 70) / Math.max(1, rect.height),
    );
  ...
  function drag(ray) {
    ...
    if (d.mode === "move") {
      const p = [...d.p];
      p[d.axis] = clamp(p[d.axis] + delta, EDIT_MIN[d.axis], EDIT_MAX[d.axis]);
