...
float paintedRegion(vec3 p,int row,int count){float d=1e5;
 for(int i=0;i<clamp(count,0,24);i++){vec4 a=patternValue(65+i,row);vec3 b=patternValue(89+i,row).xyz,ab=b-a.xyz,ap=p-a.xyz;
...
vec2 voronoiFace(vec3 p,int row,int count){float best=1e20;int nearest=0;
 for(int i=0;i<clamp(count,0,64);i++){vec3 r=p-patternValue(i+1,row).xyz;float d=dot(r,r);if(d<best){best=d;nearest=i;}}
 vec3 center=patternValue(nearest+1,row).xyz;float face=1e10;
 for(int i=0;i<clamp(count,0,64);i++){if(i==nearest)continue;vec3 s=patternValue(i+1,row).xyz,r=p-s;face=min(face,(dot(r,r)-best)/max(2.*length(s-center),1e-8));}
 return vec2(float(nearest),max(0.,face));
}
`;
export const cellDisplayGLSL =
  viewportLoopGLSL + cellCommonGLSL +
  `
vec3 cellPatternColor(vec3 p,vec3 color){
 for(int row=0;row<viewportLimit(sampleLoopLimits.z,2);row++){
