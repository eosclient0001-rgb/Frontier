import { EDIT_MIN, EDIT_MAX, CELL, SCENE_SCALE } from "./domain.js";
import { terrainNames } from "./noise-terrain.js";
...
export function createSceneEditor({
  ...
  const title = () =>
    params.preset === 4
      ? terrainNames[params.terrainForm]
      : params.preset === 3
        ? "Land plot"
        : "SDF terrain";
  function properties() {
    ...
    for (let k = 0; k < 3; k++) {
      ...
      input.disabled = locked || !p;
      input.value = p ? p[k].toFixed(2) : "";
    }
    $("#point-list").replaceChildren();
    n?.points?.forEach((p, i) => {
      const b = document.createElement("button");
      b.textContent = `${i + 1} · ${p[0].toFixed(1)}, ${p[1].toFixed(1)}, ${p[2].toFixed(1)}`;
  ...
  function row(id, name, kind, visible, onVisibility) {
    ...
    b.onkeydown = (e) => {
      const rows = [
          ...$("#scene-tree").querySelectorAll(
            ".outliner-row:not(.hidden) [role=treeitem]",
          ),
        ],
        i = rows.indexOf(b);
  ...
  function refresh() {
    ...
    const routes = nodes().filter(
      (n) => n.type === "water" && n.visible && n.points.length >= 2,
    );
    document
      .querySelectorAll("[data-route-status]")
      .forEach(
        (el) =>
          (el.textContent = routes.length
            ? `${routes.length} shared route${routes.length === 1 ? "" : "s"} · inlet at point 1 · arrow points downstream`
            : "No route yet — draw a path to place river erosion and water."),
      );
    properties();
  }
  function select(id) {
  ...
  function add(type) {
    ...
    mode = "place";
    status(
      "Click terrain upstream → downstream. Water sits above each picked point at the grid-aware clearance; edit Y for depth. Point 1 is the inlet. Finish after 2+ points.",
    );
    refresh();
  }
  $("#add-shape").onclick = () => {
    ...
    n.size = n.size.map((v) => v * SCENE_SCALE);
    n.noiseScale *= SCENE_SCALE;
    n.noiseAmount *= SCENE_SCALE;
    n.position = params.worldEnabled ? [...camera.target] : n.position;
    n.position[1] = Math.min(
      EDIT_MAX[1],
      n.position[1] + Math.max(...CELL) * 2,
    );
  ...
  $("#path-finish").onclick = () => {
    if ((node()?.points.length ?? 0) < 2) {
      toast("Place at least two points to create a route.");
      return;
    }
    mode = "edit";
    status(
      "Route ready. Point 1 emits river water; arrows show flow. Width, speed and point elevation control the same rendered water route.",
    );
    properties();
  };
  function changed() {
  ...
  $("#path-copy-water").onclick = () => {
    ...
    n.points = points.map((p) => [
      p[0],
      Math.max(
        EDIT_MIN[1],
        Math.min(EDIT_MAX[1], p[1] - depth + Math.max(0.55, CELL[1])),
      ),
      p[2],
    ]);
    n.width = Math.max(
      Math.max(...CELL) * 0.5,
      Math.min(
        Math.min(EDIT_MAX[0] - EDIT_MIN[0], EDIT_MAX[2] - EDIT_MIN[2]) * 0.5,
        width +
          2 * (source.bankSlope ?? 0.25) * Math.max(0.55, CELL[1]) +
          Math.max(0.4, CELL[0] * 0.5),
      ),
    );
    mode = "edit";
    pointIndex = 0;
  ...
  function shownPoint(n, p) {
    return n.type === "water"
      ? [p[0], p[1] + (params.waterOffset ?? 0), p[2]]
      : p;
  }
  ...
  function pointerDown(ray, x, y, rect) {
    ...
    edit(async () => {
      ...
      p = p.map((v, k) => Math.max(EDIT_MIN[k], Math.min(EDIT_MAX[k], v)));
      if (n.type === "water")
        p[1] = Math.min(EDIT_MAX[1], p[1] + Math.max(0.22, CELL[1] * 0.75));
      if (
        n.points.length &&
  ...
  function drag(ray) {
    ...
    const level = p[1] + (n.type === "water" ? (params.waterOffset ?? 0) : 0);
    if (Math.abs(ray.direction[1]) < 0.001) return;
    const t = (level - ray.origin[1]) / ray.direction[1];
    if (t <= 0) return;
    p[0] = Math.max(
      EDIT_MIN[0],
      Math.min(EDIT_MAX[0], ray.origin[0] + ray.direction[0] * t),
    );
    p[2] = Math.max(
      EDIT_MIN[2],
      Math.min(EDIT_MAX[2], ray.origin[2] + ray.direction[2] * t),
    );
    changed();
  }
  function overlay(rect) {
    ...
    for (const n of nodes()) {
      ...
      if (n.type === "water" && n.points.length) {
        const inlet = projection(shownPoint(n, n.points[0]), rect);
        if (inlet)
          html += `<text x="${inlet[0] + 9}" y="${inlet[1] + 18}" fill="${color}">INLET</text>`;
      }
      if (selected === n.id)
        n.points.forEach((p, i) => {
          const q = projection(shownPoint(n, p), rect);
  ...
  return {
    drawWater: () => add("water"),
    editWater: () => {
      const n = nodes().find((n) => n.type === "water" && n.visible);
      if (n) select(n.id);
      else add("water");
    },
