import { EDIT_MIN, EDIT_MAX } from "./domain.js";
...
export function packFlowPaths(settings = {}) {
  ...
  const data = new Float32Array(320 * 4);
  let count = 0,
    total = 0;
  for (const node of active) {
    ...
    for (let i = 0; i < points.length - 1 && count < 64; i++) {
      ...
      data.set(
        [points[0][0], points[0][2], total, points[0][1]],
        (3 + count * 3) * 4,
      );
      const offset = settings.waterOffset ?? 0;
      data.set(
        [a[1] + offset, b[1] + offset, points[0][1] + offset, 0],
        (193 + count) * 4,
      );
      along += len;
      total += len;
