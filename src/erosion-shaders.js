...
void main(){
 ...
 // Anti-Tunneling Bedload Resistance & Depth Buffer Cushion:
 // Prevents runaway single-cell pits and vertical borehole gouging.
 float depthBelowRim=max(0.,environment.x-c.y);
 float bedResistance=1.+clamp(depthBelowRim*1.4,0.,4.0);
 float critical=(.15+process.y*1.55+environment.y*strata*.55)*mix(1.,.22,loose)*bedResistance;
 float stress=sqrt(speed/max(.12,radius*.5));
 float hemisphere=2.0943951*radius*radius*radius;
 float capacity=(.02+process.w*.3)*water*(.15+.35*speed)/bedResistance;
 float detach=0.;
 if(kind==5.){
  capacity=(.025+weather.z*.3)*water;
  float unsaturated=max(0.,1.-load/max(capacity,.0001));
  detach=process.x*weather.y*unsaturated*water*hemisphere*.12*dt/(.35+process.y*bedResistance);
 }else if(kind==4.){
  ...
  // Energy dissipation limit: single-particle carve step cannot exceed fraction of voxel
  detach=process.x*max(0.,stress-critical)*hemisphere*.16*dt*sizeFactor;
 }
 // Strict CFL volume limiter per contact event: caps maximum excavation depth
 detach=min(min(detach,max(0.,capacity-load)),sums.x*VOXEL_VOLUME*.038);
