...
## Integration status (HTML side, this branch)

Reviewed against the C++ console and repaired; full record in
[`INTEGRATION.md`](INTEGRATION.md) (37 findings: units, commands, `.arc` import,
XSS hardening, keyboard map, renderer parity, roadmap).

- **Units:** the panel is millimetres everywhere; the C++ side is metres.
  The only conversion point is the `.arc` boundary (`M_TO_MM = 1000`).
- **Command line** (`arc ›`, `help` for the list): `box`/`cylinder` take legacy
  numbers or C++-style `(x,y,z)` points; `extrude` builds B-rep solids;
  `select`/`hide`/`show`/`unhide`/`delete` take `all`/`none`/names;
  `view` takes all nine views plus `fit`; `undo`/`redo` included.
- **`.arc` import:** File → open accepts C++ console scripts (curves, primitives,
  extrude/loft, move/workplane/view/selection verbs); anything without a faithful
  mapping is skipped with a warning, never faked. One undo step.
- **Keys:** `Alt+1/3/7` mirror the numpad views on laptops; `Ctrl+Y` redoes;
  ellipse is `⇧E`. The C++ hotkey chart still differs in places — see the
  parity tables in `INTEGRATION.md` before writing user docs.
- **Verify:** `node Verification/smoke.js` (432 checks) ·
  `node Verification/render.js [scene] [shade]` for offline PNGs ·
  `Scripts/publish-pages.sh` regenerates `docs/solidarc/index.html` (never edit it).
...
| Inspector | hero card (volume/area/length + B-rep stats), presence grid (Visible/Locked/Construction/Dims), transform XYZ steppers + rotation, live parameter sliders that rebuild derived figures, recipe chain, sketch curves + constraint summary, dimensions list (+ add/remove), duplicate/isolate/delete, command line (`box`, `cylinder`, `extrude`, `dim on|off`, `hide`, `show`, `unhide`, `select`, `delete`, `view`, `undo`, `redo`, `help`) + `.arc` import | `runCmd()` ← send verb to `ConsoleHost`; sliders ← `FigureRecipe` param edit |
