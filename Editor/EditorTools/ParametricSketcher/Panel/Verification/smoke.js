...
// 31. command parity + .arc import + small fixes (escapes, 1e3, undo keeps selection, guarded views)
{ M.newDoc({silent:true});
  ok(M.esc('<img src=x onerror=1>')==='&lt;img src=x onerror=1&gt;','esc() neutralises HTML in names and log lines');
  ok(M.evalExpr('1e3')===1000&&M.evalExpr('2E-2')===0.02,'scientific notation survives the expression sandbox');
  M.runCmd('box (1,2,3) 10 20 30');const B=M.doc.figures.find(f=>f.op==='box');
  ok(B&&B.params.w===10&&B.pos[0]===6&&B.pos[2]===3,'runCmd box corner form (C++ console spelling)');
  M.runCmd('box (0,0,0) (4,6,8)');ok(M.doc.figures.filter(f=>f.op==='box').pop().params.h===8,'runCmd box two-corner form');
  M.runCmd('cylinder (5,5,0) 3 10');ok(M.doc.figures.some(f=>f.op==='cylinder'&&f.params.r===3&&f.pos[0]===5),'runCmd cylinder foot form');
  M.runCmd('select all');ok(M.doc.sel.size>=3,'select all');M.runCmd('select none');ok(!M.doc.sel.size,'select none');
  M.runCmd('hide '+B.name);ok(!B.visible,'hide by name');M.runCmd('unhide all');ok(M.doc.figures.every(f=>f.visible),'unhide all');
  M.runCmd('view back');ok(M.view.az===90,'view back');M.runCmd('view bogus');ok(true,'unknown view is a hint, not a throw');
  M.runCmd('help');M.runCmd('redo');
  const sel0=[...M.doc.sel];M.runCmd('select '+B.name);M.undo();ok(M.doc.sel.size===sel0.length&&M.byId(B.id),'undo restores the selection');
  const arc=['clear','line (0,0) (1,0) --name=L1','rect (-1,-1) (1,1) --name=Plate','circle (0,0) 0.5 --name=Ring',
    'box (0,0,0) 2 1 1 --name=BoxA','extrude Plate 0.25 --name=Solid','revolve Plate 90','spline (0,0) (1,1)',
    'move BoxA (0.5,0,0)','view iso'].join('\n');
  const u0=M.doc.undoL.length;const r=M.importArc(arc);const byN=n=>M.doc.figures.find(f=>f.name===n);
  ok(r.made===5&&r.skipped===2,'.arc subset counts, got '+r.made+'/'+r.skipped);
  ok(byN('L1').params.x2===1000&&byN('Ring').params.r===500,'imported lengths convert m to mm, got '+byN('L1').params.x2);
  ok(byN('Solid').profile===byN('Plate').id&&!!M.meshOf(byN('Solid')).brep,'imported extrude takes the B-rep path');
  ok(byN('BoxA').pos[0]===1500,'imported move applies in mm');
  ok(M.doc.undoL.length===u0+1&&M.doc.undoL[u0]==='import .arc','import is a single undo step');
  M.undo();ok(!byN('L1'),'import undoes cleanly');M.sub_.sel.clear();M.doc.sel.clear(); }
