...
function frameOf(sk){
  // base plane frame + the shape's out-of-plane placement (Z lift, X/Y tilt about its centre)
  const F=PLANES[sk.plane]||PLANES.XY;
  const p=sk.pose;
  if(!p||(!p.tz&&!p.rx&&!p.ry))return F;
  const rot=(q,ax,a)=>{ // Rodrigues rotation of q about unit axis ax
    const c=Math.cos(a),s=Math.sin(a),d=V.dot(ax,q);
    return V.add(V.add(V.mul(q,c),V.mul(V.cross(ax,q),s)),V.mul(ax,d*(1-c)));
  };
  const R=q=>rot(rot(q,F.v,p.ry||0),F.u,p.rx||0);
  const u=R(F.u),v=R(F.v),w=R(F.w);
  const c=shapeBBox(sk).c;                                   // pivot = shape centre
  const A=V.add(F.o,V.add(V.mul(F.u,c[0]),V.mul(F.v,c[1]))); // its world position
  const o=V.add(V.sub(A,V.add(V.mul(u,c[0]),V.mul(v,c[1]))),V.mul(F.w,p.tz||0));
  return {o,u,v,w};
}
...
function rebuildBody(b){
  ...
  const zs=(sks[0].pose&&sks[0].pose.sz)||1; // shape's Z scale stretches the extrusion
  let S=K.extrude(regs[b.region||0]||regs[0],frameOf(sks[0]),b.h*zs,{caps:!b.shell});
  // transform placement
...
function bodySolid(b){
  ...
  const stamp=JSON.stringify([b.h,b.xf,b.edits,b.shell,bodySources(b).map(s=>[s.segs,s.pose||0])]);
...
function shapePose(sk){
  // rot/sx/sy live in the plane and are baked into segs; tz/rx/ry/sz are the out-of-plane
  // placement (Z lift, X/Y tilt, Z scale — applied via frameOf / at extrude time)
  const def={rot:0,sc:1,sx:1,sy:1,sz:1,tz:0,rx:0,ry:0};
  if(!sk.pose)sk.pose=def;
  else for(const k in def)if(sk.pose[k]==null)sk.pose[k]=def[k];
  return sk.pose;
}
function applyShapeDelta(sk,d){ // d:{du,dv,rot(rad),ds | dsu,dsv} — rot/scale about the bbox centre
  const c=shapeBBox(sk).c;
  const cs=Math.cos(d.rot||0),sn=Math.sin(d.rot||0);
  const su=d.dsu!=null?d.dsu:(d.ds||1), sv=d.dsv!=null?d.dsv:(d.ds||1);
  const uni=Math.abs(su-sv)<1e-12;
  const map=(x,y)=>{
    let px=x-c[0],py=y-c[1];
    const rx=(px*cs-py*sn)*su,ry=(px*sn+py*cs)*sv;
  ...
  for(const s of sk.segs){
    if(s.full){
      ...
      for(let i=0;i<N;i++){
        ...
        const at=t=>map(s.cx+s.r*Math.cos(t),s.cy+s.r*Math.sin(t));
        const A=at(t0),B=at(t1),M=at(tm);
        out.push({x0:A[0],y0:A[1],x1:B[0],y1:B[1],b:bulge3(A,M,B)});
      }
      continue;
    }
    const a=map(s.x0,s.y0),b=map(s.x1,s.y1);
    if(!s.b||uni){out.push({...s,x0:a[0],y0:a[1],x1:b[0],y1:b[1]});continue;} // bulge invariant under rigid+uniform
    // bulged arc under non-uniform scale → split into ≤30° pieces, refit each through 3 mapped points
    const sw=4*Math.atan(s.b),{cx,cy,r,a0}=KM.arcOf(s);
    const n=Math.max(1,Math.ceil(Math.abs(sw)/(Math.PI/6)));
    for(let i=0;i<n;i++){
      const t0=a0+sw*i/n,t1=a0+sw*(i+1)/n,tm=(t0+t1)/2;
      const at=t=>map(cx+r*Math.cos(t),cy+r*Math.sin(t));
      const A=at(t0),B=at(t1),M=at(tm);
      out.push({x0:A[0],y0:A[1],x1:B[0],y1:B[1],b:bulge3(A,M,B)});
    }
  }
...
function drawPlaneFill(plane,sks){
  // Every filled shape paints its OWN full fill — overlaps stay filled (they just deepen
  // slightly). Geometry is only combined when the user runs an explicit boolean/join.
  // Even-odd applies only WITHIN one figure, so a joined shape keeps its holes.
  ctx.save();
  for(const sk of sks){
    if(sk.fill===false)continue;
    const F=frameOf(sk); // honors the shape's 3-D placement (Z offset / tilt)
    const P2=(x,y)=>project(V.add(F.o,V.add(V.mul(F.u,x),V.mul(F.v,y))));
    const regs=profileOf(sk);
    if(!regs.length)continue;
    const sel=selection.some(s=>s.body===sk.id&&(s.type==='sketch'||s.type==='curve'));
    ctx.fillStyle=sel?'rgba(255,180,84,.22)':'rgba(120,200,255,.16)';
    ctx.beginPath();
    let any=false;
...
function renderInspector(){
  ...
  if(f.kind==='body'){
    ...
    const triple=(lab,unit,cells)=>`
      <div class="ctl"><div class="ctl-top"><span class="lab">${lab}</span><span class="val" style="font-size:10px">${unit}</span></div>
        <div class="ctl-top" style="gap:6px">${cells.map(([ax,k,val,step])=>`
          <span style="flex:1;display:flex;align-items:center;gap:4px"><span class="lab" style="font-size:10px;color:var(--t3)">${ax}</span>
          <input class="pin" data-p="${k}" type="number" step="${step}" value="${val}"
            style="width:100%;min-width:0;background:rgba(255,255,255,.05);border:1px solid var(--stroke);border-radius:6px;color:var(--text);font:inherit;font-size:11.5px;padding:3px 6px"></span>`).join('')}
        </div></div>`;
    el.innerHTML=`
      <div class="hero" style="--acc:var(--cyan)">
    ...
    el.querySelectorAll('.pin').forEach(inp=>{
      inp.onkeydown=ev2=>{
        ...
        else if(k==='Z')pose2.tz=v;                              // lift off the sketch plane
        else if(k==='rotX')pose2.rx=v*Math.PI/180;               // tilt about the plane's X
        else if(k==='rotY')pose2.ry=v*Math.PI/180;               // tilt about the plane's Y
