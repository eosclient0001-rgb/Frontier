...
## Library, Construct catalogue → Outliner

The Outliner has a **Library · all plants** group listing every plant in the catalogue (click = make
current, **+** / double-click = add to scene) above the **Scene** groups of plants you've generated.
...
| `Verification/plant_smoke.mjs` | `node Verification/plant_smoke.mjs` — 25 plants × 20 seeds: single connected mesh, finite positions, valid indices, height range, colour attribute present, no UVs, determinism, uniqueness. |
