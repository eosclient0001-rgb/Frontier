...
scene.add(skel.rig);
const decorations = buildDecorations(skel);
...
renderer.domElement.addEventListener('pointerdown', (e) => { downX = e.clientX; downY = e.clientY; downT = performance.now(); }, true);
// a plain CLICK on the ground (not a drag, not on the ball) lobs the ball there
renderer.domElement.addEventListener('pointerup', (e) => {
  if (!preyGroup.visible || dragging || e.button !== 0) return;
  if (Math.hypot(e.clientX - downX, e.clientY - downY) > 6 || performance.now() - downT > 550) return;
  mouse.set((e.clientX / window.innerWidth) * 2 - 1, -(e.clientY / window.innerHeight) * 2 + 1);
  ray.setFromCamera(mouse, camera);
  const hit = new Vector3();
  if (!ray.ray.intersectPlane(dragPlane, hit)) return;
  const x = hit.x, z = hit.z;
  const dx = x - prey.pos.x, dz = z - prey.pos.z, d = Math.hypot(dx, dz);
  const T = Math.min(2.2, Math.max(0.5, 0.45 + d / 13));
  prey.held = false;
  prey.vel.set(dx / T, Math.max(1.5, 0.5 * 9.81 * T), dz / T).clampLength(0, 14);
}, true);
...
window.__trex = { skel, anim, scene, camera, renderer, hunter, prey, decorations };
frame();
