...
export class Hunter {
  constructor(anim, skel, prey) {
    ...
    anim.on((ev, a) => {
      ...
      if (ev === 'end' && (a.type === 'bite' || a.type === 'tailSwipe')) {
        this.cooldown = a.hit ? 1.2 : 0.55;
        if (!a.hit) this.emit('miss', { kind: a.type === 'bite' ? 'bite' : 'tail' });
      }
  ...
  update(dt) {
    ...
    if (this.cooldown <= 0 && A.speed < 3.2) {
      ...
      if (A.speed < 0.6 && this.tailStrikeZone(L)) {
   ...
   */
  tailStrikeZone(L) {
    // measured swept arc (tests/tail-sweep map): prey centre 4.6-5.5 m from
    // the hips, 0°-60° off straight-behind, below ~1.4 m
    const back = -L.fwd, side = Math.abs(L.lat);
    const r = Math.hypot(back, side);
    const off = Math.atan2(side, back) / DEG;
    return back > 0 && r > 4.6 && r < 5.5 && off < 60 && L.h < 1.4;
  }
  ...
  _resolveTail(a) {
    ...
    for (let i = 4; i < tail.length; i++) {
      ...
      if (_v.distanceTo(this.prey.pos) < this.prey.radius + r) {
        a.hit = true;
        this.stats.hits++;
