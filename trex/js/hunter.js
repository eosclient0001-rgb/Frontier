...
export class Hunter {
  constructor(anim, skel, prey) {
    ...
    this.stats = { bites: 0, tail: 0, hits: 0 };
    ...
    anim.on((ev, a) => {
      ...
      if (ev === 'end' && (a.type === 'bite' || a.type === 'tailSwipe')) {
        // Do not repeat a tail strike forever when the prey is just outside
        // the measured arc; turn back into a bite approach after one miss.
        this.cooldown = a.hit ? 1.2 : (a.type === 'tailSwipe' ? 2.4 : 0.55);
        if (!a.hit) {
          if (a.type === 'tailSwipe') this.tailMissed = true;
          this.emit('miss', { kind: a.type === 'bite' ? 'bite' : 'tail' });
        }
      }
  ...
  update(dt) {
    ...
    if (this.haveLastPrey && this.lastPreyPos.distanceToSquared(this.prey.pos) > 16) this.tailMissed = false;
    ...
    const biteReach = L.fwd > 4.4 && L.fwd < 7.0 && Math.abs(L.lat) < 1.7 && L.h < 3.4;
    if (this.cooldown <= 0 && A.speed < 3.2) {
      ...
      if (A.speed < 0.6 && !this.tailMissed && this.tailStrikeZone(L)) {
    ...
    if (this.reposition) {
      ...
      R.t += dt;
      if (R.t > 9 || (R.phase === 'walk' && L.dist > 7.5)) this.reposition = null;
      else {
        if (R.phase === 'turn') {
          A.setTargetSpeed(0);
          A.setTurn(R.dir);
          if (Math.abs(L.ang) > 110 * DEG) R.phase = 'walk';
        } else {
          A.setTargetSpeed(1.6);
          A.setTurn(0);
        }
    ...
    else if (L.dist > 12) speed = clamp(2.2 + (L.dist - 12) * 0.5, WALK_SPEED, RUN_SPEED);
    else if (L.fwd > 7.0) speed = clamp(0.8 + (L.fwd - 7) * 0.35, 0.8, WALK_SPEED);
    else speed = 0;
  ...
  tailStrikeZone(L) {
    ...
    return back > 0 && r > 4.0 && r < 5.8 && off < 64 && L.h < 1.4;
  }
  ...
  _resolveTail(a) {
    ...
    for (let i = 4; i < tail.length; i++) {
      ...
      const swept = 0.34 + 0.10 * (i / tail.length);
