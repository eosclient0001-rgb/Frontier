...
export const ACTION_DEFS = {
  ...
  headShove: { dur: 1.5, lockFeet: false },
...
export class Animator {
  ...
  startAction(type, opts = {}) {
    ...
    if (type === 'bite') {
      // Manual bites cycle four feeding strategies — three orthal patterns plus
      // an angled skull-cock. Hunt bites keep the deterministic 3-cycle the
      // catch guarantee is tuned for. Pin one with { variant: 0-3 }.
      a.variant = opts.variant ?? (this.biteSerial++ % (opts.target ? 3 : 4));
    ...
    if (type === 'tailSwipe' || type === 'headShove') a.side = opts.side ?? 1;
    this.action = a;
    this.emit('start', a);
    return true;
  }
  ...
  get gaitLabel() {
    if (this.action) {
      const n = {
        bite: 'Bite', headShove: 'Head shove', tailSwipe: 'Tail swipe', roar: 'Roar', look: 'Look around',
  ...
  _evalAction(dt) {
    ...
    if (a.type === 'bite') {
      ...
      // the gape completes before the strike lands (reads as a snap, not a nibble)
      const close = smoothstep(v === 1 ? 0.45 : 0.40, v === 2 ? 0.62 : 0.64, t);
      ...
      A.headPitch = (14 * wind - (v === 3 ? 5.5 : 3.5) * contact + (v === 2 ? 6 : v === 1 ? -2 : 2) * pull) * DEG;
      A.neckPitch = (a.hasTarget ? 4 * wind : 4 * wind - 11 * strike + 3 * pull) * DEG;
      ...
      // variant 3 — angled bite: skull cocks and rolls away in the windup,
      // then unwinds through the gnash (visible skull rotation, unlike orthal bites)
      A.headRoll += v === 3 ? (16 * wind - 20 * contact + 6 * pull) * DEG : 0;
      A.headYaw += v === 3 ? -8 * DEG * wind : 0;
    ...
    } else if (a.type === 'headShove') {
      // A strike delivered by the head swinging sideways through the target.
      // Jaw stays essentially shut; the neck sweeps low and the skull carves
      // a lateral arc. The hips brace rather than fold; feet keep stepping if needed.
      const s = a.side;
      const windT = smoothstep(0.04, 0.24, t) * (1 - smoothstep(0.30, 0.42, t));   // quick cock away
      const sw = smoothstep(0.26, 0.60, t);                                        // the sweep itself
      const rec = smoothstep(0.88, 1.28, t);
      const env = smoothstep(0, 0.12, t) * (1 - rec);
      const lowTarget = a.hasTarget && this.aimTarget
        ? clamp((1.30 - this.aimTarget.y) * 1.15, 0, 1.0)
        : 0;
      A.drop = 0.13 * lowTarget * smoothstep(0.16, 0.40, t) * (1 - smoothstep(0.9, 1.3, t));
      A.pitch = (1.5 * windT - 3.0 * sw * lowTarget) * DEG;        // brace, don't fold
      // head/neck arc: start cocked to the opposite side, sweep through centre and past it
      A.headYaw += s * lerp(30, -38, sw) * DEG * env;
      A.neckYaw += s * lerp(16, -20, sw) * DEG * env;
      A.headRoll += s * (-9 * windT + 7 * sw) * DEG * env;
      A.headPitch += (7 * windT - 6 * sw * (0.4 + lowTarget)) * DEG;
      A.neckPitch += (3 * windT - 4 * sw * lowTarget) * DEG;
      A.aimW = a.hasTarget ? Math.max(windT, sw) * (1 - rec) : 0;
      A.lungeW = a.hasTarget ? sw * 0.6 : 0;
      A.jaw = (0.22 * windT + 0.045 * sw) * (1 - rec);
      A.arms = sw * 0.5;
      A.speedCap = t > 0.4 ? 1.2 : Infinity;
      if (t > 0.42 && t < 0.98) this.emit('headSweep', a);
