...
export class Animator {
  ...
  _evalAction(dt) {
    ...
    if (a.type === 'bite') {
      ...
      const wind = ease(t / 0.34);
      const close = smoothstep(0.40, 0.68, t);
      const contact = smoothstep(0.58, 0.73, t);
      const pull = smoothstep(0.70, 1.06, t) * (1 - smoothstep(1.08, 1.42, t));
      const rec = ease((t - 1.20) / 0.58);
      const strike = smoothstep(0.30, 0.62, t) * (1 - rec);
      ...
      A.pitch = (2.0 * wind - 4.5 * strike + 1.4 * pull) * DEG;
      A.drop = 0.10 * lowTarget * contact + (a.hasTarget ? 0 : 0.12 * strike);
      A.surge = 0.14 * wind + 0.16 * pull;
      A.lungeW = a.hasTarget ? strike : 0;
      A.headPitch = (10 * wind - 2.5 * contact + 2 * pull) * DEG;
      ...
      A.jaw = 1.02 * wind * (1 - close) + 0.035 * close * (1 - rec);
  ...
  _updatePelvis(g, dt) {
    ...
    _e.set(pitch, yaw, rollRaw - idle.w * idle.shift * 0.6 + A.roll + pivotRoll, 'YXZ');
  ...
  _updateAxial(g, dt) {
    ...
    const lookYaw = (I.look.x * I.w * (1 - noLook)) * DEG + this.yawRate * 0.55 + sniffYaw;   // look into turns
