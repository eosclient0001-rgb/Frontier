...
| 17 | D9 GPU refit / GPU build kernels (wide-AABB refit; H-PLOC for topology changes) | ⚠️ | 80 % | **Both kernels written and in the compile gate**: `BlasRefit.slang` (leaf rewrite + one dispatch per level, deepest first) and `BlasBuild.slang` (Morton prepass · per-level octant partition, which is also the sort · childBase scan · emit · leaf runs) — 15/15 lowered by `CheckShaders.sh`, and §⑩ pins B1–B40 of their text to the CPU mirror, which §⑨ measures: 63 854 triangles → 17 835 nodes in 10 levels, every triangle reachable, 21.3 ms build, refit level-ordered ≡ D8's sweep byte for byte (4 827 nodes, 31 927 tris, 0 diffs). 163 gates, 0 failed. ⚠️ The build is the LBVH-shaped variant, not H-PLOC's cluster merge (64 % of the wide slots stay empty — the traversal pays, the format tolerates), and it has never been compiled by `slangc` or run on a device. Left: **one GPU run** + the host wiring |
