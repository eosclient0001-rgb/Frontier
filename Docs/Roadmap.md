...
| 5 | Indirect coverage 16 % → 100 % (replay + shift mapping) | ❌ | 15 % | **The gap is measured, not assumed** (report §14.5): of the 84 % of surface pixels the pool misses, **69 % are sky escapes** (no first-bounce vertex exists to reconnect from), 4.5 % are glass/SSS (out of scope by design), 2.9 % are emitter hits, 1.2 % degenerate. The temporal half is alive — it merges 92.7 % of the attempts it gets but is asked on only ~25 % of covered pixels, because the vertex is a per-frame coin flip from a fresh BSDF draw. Next step: land the receiver-side shift (vertex payload in the reservoir + `f·cos·G·V` connect term) behind its own switch and read it against a converged reference |
