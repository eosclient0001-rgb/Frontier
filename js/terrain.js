...
export class TerrainView {
  constructor(container, { onProbe, onFps, onSculptStroke } = {}) {
    this.onSculptStroke = onSculptStroke;
    ...
    this.sculptRadius = 6.0;
    this.isSculpting = false;
    this.lastSculptPt = null;

    // Pointer events for sculpting on canvas
    const dom = renderer.domElement;
    dom.addEventListener('pointerdown', (e) => {
      if (this.sculptMode && e.button === 0) {
        this.isSculpting = true;
        this.controls.enabled = false;
        const hit = this.raycastTerrain();
        if (hit) {
          this.lastSculptPt = { x: hit.x, z: hit.z, elev: hit.elev };
          if (this.onSculptStroke) {
            this.onSculptStroke({ p0: this.lastSculptPt, p1: this.lastSculptPt, isStart: true });
          }
        }
      }
    });
    ...
    window.addEventListener('pointerup', () => {
      if (this.isSculpting) {
        ...
        if (this.onSculptStroke) {
          this.onSculptStroke({ isEnd: true });
        }
      }
    });
  ...
  setSculptMode(enabled) {
    this.sculptMode = !!enabled;
    this.controls.enabled = !this.sculptMode;
    if (this.brushRing) this.brushRing.visible = this.sculptMode;
    this.renderer.domElement.style.cursor = this.sculptMode ? 'crosshair' : 'default';
  }
  ...
  raycastTerrain() {
    if (!this.terrainMesh || !this.probe) return null;
    ...
    return { x, z, elev: h[idx], slope, point: hits[0].point };
  }

  doProbe() {
    if (!this.terrainMesh || !this.probe) return;
    const hit = this.raycastTerrain();
    if (hit) {
      if (this.brushRing && this.sculptMode) {
        this.brushRing.position.copy(hit.point);
        this.brushRing.position.y += 0.08;
        this.brushRing.visible = true;
      }
      this.onProbe && this.onProbe({ x: hit.x, z: hit.z, elev: hit.elev, slope: hit.slope });

      if (this.isSculpting && this.onSculptStroke) {
        const cur = { x: hit.x, z: hit.z, elev: hit.elev };
        const p0 = this.lastSculptPt || cur;
        this.onSculptStroke({ p0, p1: cur, isStart: false });
        this.lastSculptPt = cur;
      }
    } else {
      if (this.brushRing && this.sculptMode) this.brushRing.visible = false;
