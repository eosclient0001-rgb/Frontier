import { applySculptDab, applySculptStroke, SCULPT_TOOLS } from './sculpt.js';
/* ============================================================
...
const P = {
  ...
  passes: {
    fluvial: true, particles: true, debris: true, hydraulic: false, rivers: false,
    braid: true, micro: true, diffuse: false,
  },
  // sculpt
  sculptTool: 'ridge', sculptRadius: 6.0, sculptStrength: 0.35, sculptFalloff: 2.0,
  // splats
...
const view = new TerrainView(viewport, {
  ...
  onSculptStroke: ({ p0, p1, isStart, isEnd }) => {
    if (isEnd) {
      if (S.h) {
        liveRebuild();
        updateTerrainStats();
    ...
    applySculptStroke({
      ...
      p0, p1,
      tool: P.sculptTool,
      radius: P.sculptRadius,
      strength: P.sculptStrength,
      falloff: P.sculptFalloff,
      targetH: S._sculptTargetH || p0.elev,
      seed: P.seed,
      erosionMap: S.maps ? S.maps.erosionMap : null,
      depositMap: S.maps ? S.maps.depositMap : null,
    });
    // Throttle mesh vertex position update during drag
    rebuildMesh();
  },
});
...
// ---- Sculpting section (SDF Interactive Sculpting) ----
{
  const b = mkSection(left, 'SDF SCULPTING', 'interactive 3D brush on SDF surface');
  
  const toolRow = document.createElement('div');
  toolRow.className = 'toolgrid';
  C.sculptToolBtns = {};
  
  const tools = [
    { id: 'view', label: 'View / Orbit', desc: 'Navigate and inspect scene' },
    { id: 'ridge', label: 'Ridge / Raise', desc: 'Build ridges, hills and peaks' },
    { id: 'dent', label: 'Dent / Carve', desc: 'Carve valleys, gullies and riverbeds' },
    { id: 'smooth', label: 'Smooth', desc: 'Relax sharp rills and harsh edges' },
    { id: 'flatten', label: 'Flatten', desc: 'Plateau tool: level surface to contact elevation' },
    { id: 'texture', label: 'Rock Texture', desc: 'Stamp micro-rock roughness onto terrain' },
  ];

  tools.forEach(t => {
    const btn = document.createElement('button');
    btn.className = 'sculptbtn' + (t.id === (P.sculptMode ? P.sculptTool : 'view') ? ' on' : '');
    btn.dataset.id = t.id;
    btn.textContent = t.label;
    btn.title = t.desc;
    btn.addEventListener('click', () => {
      Object.values(C.sculptToolBtns).forEach(b => b.classList.remove('on'));
      btn.classList.add('on');
      if (t.id === 'view') {
        P.sculptMode = false;
        view.setSculptMode(false);
      } else {
        P.sculptMode = true;
        P.sculptTool = t.id;
        view.setSculptMode(true);
        view.setSculptRadius(P.sculptRadius);
      }
    });
    toolRow.appendChild(btn);
    C.sculptToolBtns[t.id] = btn;
  });
  b.appendChild(toolRow);

  C.sculptRadius = mkSlider(b, {
    id: 'sculptRadius', label: 'Brush radius', min: 1, max: 25, step: 0.5, value: P.sculptRadius, unit: ' m'
  });
  C.sculptStrength = mkSlider(b, {
    id: 'sculptStrength', label: 'Brush strength', min: 0.05, max: 1.0, step: 0.05, value: P.sculptStrength
  });
  C.sculptFalloff = mkSlider(b, {
    id: 'sculptFalloff', label: 'Brush falloff', min: 1.0, max: 4.0, step: 0.2, value: P.sculptFalloff
  });

  const note = document.createElement('div');
  note.className = 'dim small';
  note.style.marginTop = '6px';
  note.innerHTML = 'Left-click + drag on terrain to sculpt in real time.<br>Select <b>View / Orbit</b> or hold Space/Alt to orbit.';
  b.appendChild(note);
}
...
bindChange('sculptRadius', (v) => { view.setSculptRadius(v); });
bindChange('sculptStrength');
bindChange('sculptFalloff');
