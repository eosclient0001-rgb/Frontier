...
import {Whitewater,FOAM,SPRAY} from './whitewater-core.mjs?v=20260918-startup4';
import {withSurfaceFoam} from './foam-surface.mjs?v=20260918-startup4';
import {WhitewaterView} from './whitewater-render.mjs?v=20260918-startup4';
...
catch(error){throw new Error('WebGL 2 could not start. Enable browser hardware acceleration, then reload. '+error.message);}
renderer.setPixelRatio(Math.min(devicePixelRatio,1));
renderer.shadowMap.enabled=true;renderer.shadowMap.type=THREE.PCFSoftShadowMap;
renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=.95;
const world=new THREE.Scene();world.background=new THREE.Color('#101c26');world.fog=new THREE.Fog('#101c26',25,65);
const whitewater=new Whitewater(512);whitewater.setLimit(256);
const whitewaterView=new WhitewaterView(512,whitewater.foam);world.add(whitewaterView.group);whitewaterView.crownsEnabled=false;
const pendingSplashes=[];let splashCooldown=0;
let performanceMode='balanced',opticsDirty=true,opticsReady=false,opticsFrames=3,lastOptics=-Infinity,scenePasses=0,fxCPU=0,frameFX=0;
const profile={fast:{dpr:.85,refract:384,reflect:256},balanced:{dpr:1,refract:640,reflect:384},full:{dpr:1.5,refract:850,reflect:600}};
...
home();
world.add(new THREE.HemisphereLight('#c9eaf5','#183443',1.3));
const sun=new THREE.DirectionalLight('#e9f5ff',2.8);sun.position.set(-7,14,6);sun.castShadow=true;sun.shadow.mapSize.set(1024,1024);sun.shadow.autoUpdate=false;sun.shadow.needsUpdate=true;Object.assign(sun.shadow.camera,{left:-9,right:9,top:9,bottom:-9,near:1,far:40});sun.shadow.normalBias=.035;sun.shadow.bias=-.0001;sun.shadow.radius=3;world.add(sun);
const fill=new THREE.DirectionalLight('#79b9d2',1.6);fill.position.set(8,6,-5);world.add(fill);
...
sky.addColorStop(0,'#304c66');sky.addColorStop(.35,'#7cabbc');sky.addColorStop(.5,'#cbeaf1');sky.addColorStop(.57,'#294449');sky.addColorStop(1,'#102831');ec.fillStyle=sky;ec.fillRect(0,0,1024,512);
for(let i=0;i<30;i++){const a=i*36;ec.fillStyle=i%2?'#29464d':'#3d6066';ec.beginPath();ec.ellipse(a,295+Math.sin(i*2)*24,20+Math.sin(i)*12,40+Math.cos(i)*20,0,0,Math.PI*2);ec.fill();}
...
const limestone=material('#3a4e59'),stoneLight=material('#607785'),stoneDark=material('#344b51'),earth=material('#263944');
...
box(200,.1,200,material('#101c26'),0,-.76,0,world);
function buildBasin(){
...
orbit.addEventListener('change',()=>{renderDirty=true;opticsDirty=true;});
...
const waterMaterial=new THREE.ShaderMaterial({
  uniforms:{...whitewaterView.surfaceUniforms,uTime:{value:0},uEnvironment:{value:environment},uLevel:{value:level},
  ...
  vertexShader:waterVertex,fragmentShader:withSurfaceFoam(waterFragment),side:THREE.DoubleSide
});
...
function paintRange(input){const p=(Number(input.value)-Number(input.min))/(Number(input.max)-Number(input.min))*100;input.style.background=`linear-gradient(to right,#7bcbd0 ${p}%,#293b47 ${p}%)`;}
...
function reset(){
  whitewater.reset();pendingSplashes.length=0;splashCooldown=0;opticsDirty=true;
  ...
  whitewater.foam.configure(solver);release();duckState.reset(level);syncDuck(0);rebuildWater();updateWater();updateLilies(0);updateMetrics();
}
function updateMetrics(){
  const effects=whitewater.stats();$('foamCount').textContent=effects.foamArea.toFixed(2)+' m²';$('sprayCount').textContent=effects.spray.toLocaleString();$('particleUsage').textContent=effects.total+' / '+effects.limit;$('effectsMeter').style.width=(effects.total/effects.limit*100)+'%';$('wwStatus').textContent=effects.foamResolution.join(' × ')+' coverage field · 20 Hz · 0 foam draws';$('fxCost').textContent=fxCPU.toFixed(2)+' ms';$('passCount').textContent=scenePasses+' scene pass'+(scenePasses===1?'':'es');
...
canvas.addEventListener('pointerdown',e=>{
  ...
  if(tool==='splash')queueSplash(p.x,p.z,strength*1.5);
  if(tool==='duck'){
...
canvas.addEventListener('pointermove',e=>{
  ...
  if(tool==='splash'&&lastHit&&p.distanceTo(lastHit)>.6){queueSplash(p.x,p.z,strength);lastHit=p;}
...
for(const id of ['wave','damping','strength','level']){
  const el=$(id),update=()=>{const val=Number(el.value);if(id==='wave')solver.speed=val;if(id==='damping')solver.damping=val;if(id==='strength')strength=val;if(id==='level'){level=val;updateBed();if(!running){duckState.y=level+solver.sample(duckState.x,duckState.z);duckState.vy=0;}}
    $(id+'Value').textContent=val.toFixed(id==='wave'?1:2)+(id==='wave'?' m/s':id==='level'?' m':'');const p=(val-Number(el.min))/(Number(el.max)-Number(el.min))*100;el.style.background=`linear-gradient(to right,#7bcbd0 ${p}%,#293b47 ${p}%)`;};el.oninput=update;update();
}
$('capture').onclick=()=>{whitewaterView.update(whitewater,solver,level,waterStyle);renderScene(true);canvas.toBlob(blob=>{if(!blob){toast('Unable to save this snapshot.');return;}const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='ripple-whitewater-'+Date.now()+'.png';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);toast('A little piece of the pond, saved.');});};
...
window.addEventListener('keydown',e=>{if(['INPUT','SELECT','BUTTON'].includes(document.activeElement.tagName)||$('about').open)return;if(e.code==='Space'){e.preventDefault();togglePlay();}if(e.key.toLowerCase()==='r')reset();if(['1','2','3','4','5'].includes(e.key))selectTool(['orbit','ripple','lily','duck','splash'][Number(e.key)-1]);});
...
function queueSplash(x,z,power=1){
  if(!running){toast('Press Play before making a splash.');return;}
  if(solver.isWet(x,z,.12)&&pendingSplashes.length<8)pendingSplashes.push({x,z,power});
}
$('demoSplash').onclick=()=>{queueSplash(-.8,.1,1.25);toast('Splash queued · watch the droplets land as foam.');};
$('clearWhitewater').onclick=()=>{whitewater.reset();pendingSplashes.length=0;updateMetrics();renderDirty=true;toast('Effects cleared. The water simulation is unchanged.');};
$('showFoam').onchange=e=>{whitewater.foamEnabled=e.target.checked;if(!e.target.checked)whitewater.clearKind(FOAM);renderDirty=true;updateMetrics();};
$('showSpray').onchange=e=>{whitewater.sprayEnabled=e.target.checked;if(!e.target.checked){whitewater.clearKind(SPRAY);whitewater.crowns.length=0;}renderDirty=true;updateMetrics();};
$('showCrowns').onchange=e=>{whitewaterView.crownsEnabled=e.target.checked;renderDirty=true;};
$('crestEmission').onchange=e=>whitewater.crestEnabled=e.target.checked;
$('particleBudget').onchange=e=>{whitewater.setLimit(Number(e.target.value));renderDirty=true;updateMetrics();};
for(const [id,key] of [['whitewaterAmount','amount'],['foamLife','foamLife']]){
  const el=$(id);el.oninput=()=>{whitewater[key]=Number(el.value);$(id+'Value').textContent=Number(el.value).toFixed(id==='foamLife'?0:1)+(id==='foamLife'?' s':'×');paintRange(el);};el.oninput();
}
function resizeRenderer(){
  const r=viewport.getBoundingClientRect(),p=profile[performanceMode];renderer.setPixelRatio(Math.min(devicePixelRatio,p.dpr));renderer.setSize(r.width,r.height,false);camera.aspect=r.width/r.height;camera.fov=camera.aspect<1.1?49:39;camera.updateProjectionMatrix();
  const rw=Math.min(p.refract,Math.round(r.width));refractionTarget.setSize(rw,Math.round(rw/r.width*r.height));const mw=Math.min(p.reflect,rw);reflectionTarget.setSize(mw,Math.round(mw/r.width*r.height));
  renderDirty=true;opticsDirty=true;opticsReady=false;
}
$('renderMode').onchange=e=>{performanceMode=e.target.value;const full=performanceMode!=='fast';waterMaterial.uniforms.uUsePlanar.value=full?1:0;waterMaterial.uniforms.uUseRefraction.value=full?1:0;resizeRenderer();};
new ResizeObserver(resizeRenderer).observe(viewport);
applyWaterStyle('ocean');$('waterMaterial').value='ocean';reset();
// One opening splash demonstrates the new layer; there is no continuous emitter.
queueSplash(-.8,.1,1.1);
...
function renderScene(force=false){
  ...
  const now=performance.now();opticsFrames++;
  if(!opticsReady&&performanceMode==='fast'){renderer.initRenderTarget(reflectionTarget);renderer.initRenderTarget(refractionTarget);opticsReady=true;}
  const capture=!opticsReady||(performanceMode!=='fast'&&(force||opticsDirty||performanceMode==='full'||opticsFrames>=3&&now-lastOptics>=50));
  scenePasses=capture?3:1;
  if(capture){
  ...
  const visibleWire=wire.visible;water.visible=false;wire.visible=false;whitewaterView.group.visible=false;
  ...
  water.visible=true;wire.visible=visibleWire;whitewaterView.group.visible=true;
  opticsReady=true;opticsDirty=false;opticsFrames=0;lastOptics=now;
  }
  renderer.render(world,camera);
  if(!window.pondFirstFrame){window.pondFirstFrame=true;window.dispatchEvent(new Event('pond-ready'));}
}
function animate(now){
  const elapsed=Math.max(0,Math.min((now-last)/1000,.1));last=now;frameFX=0;
  if(running){
    refineCheck+=elapsed;
    ...
    const advanced=solver.advance(elapsed,32,dt=>{
      ...
      if(pendingSplashes.length&&splashCooldown===0){
        const splash=pendingSplashes.shift();solver.disturb(splash.x,splash.z,Math.min(1.2,splash.power),.33);
        whitewater.burst(solver,splash.x,splash.z,level,splash.power,whitewaterView.crownsEnabled);splashCooldown=.22;
      }
      const wakesBefore=duckState.wakes;duckState.update(solver,dt,level);
      if(duckState.wakes>wakesBefore)whitewater.wake(solver,duckState,level);
      const fxStart=performance.now();whitewater.update(solver,dt,level);frameFX+=performance.now()-fxStart;
