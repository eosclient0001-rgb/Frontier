...
test("deterministic 3D formation with different seeded geometry", () => {
  ...
  assert.notEqual(
    ...
    baseSDF(-15, 8, 3, { ...defaults, terrainSeed: 7309 }),
  );
  assert(noise(1.1, 2.2, 3.3) >= 0 && noise(1.1, 2.2, 3.3) <= 1);
});
test("finite noise volume contains an editable XYZ solid and surrounding air", () => {
  volume = generateVolume(defaults);
  assert.equal(volume.length, SIZE.reduce((a, b) => a * b) * 4);
  assert(sampleVolume(volume, [-5, 4, 0]) < 0, "solid noise terrain");
  assert(sampleVolume(volume, [-10, 6, 0]) > 0, "air outside mountain");
...
test("sculpting can carve inside rock and build in empty space", () => {
  ...
  const built = sculptVolume(volume, [-10, 6, 0], 1.5, "add");
  assert(sampleVolume(built, [-10, 6, 0]) < -1);
  const smoothed = sculptVolume(carved, [-4, 4, 0], 2, "smooth");
...
test("picking intersects the visible 3D surface and misses empty rays", () => {
  const p = raycastVolume(volume, [0, 30, 0], [0, -1, 0]);
  assert(p && p[1] > 7 && p[1] < 20);
  assert.equal(raycastVolume(volume, [40, 30, 40], [0, 1, 0]), null);
});
test("noise and blank plot bases are bounded and finite", () => {
  for (const preset of [3, 4])
    for (const xyz of [
      [0, 0, 0],
      [0, 10, 0],
      [-8, 6, 5],
