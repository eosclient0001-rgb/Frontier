  ...
  await page.evaluate(async () => {
    ...
    const canvas = document.querySelector("canvas"),
      p = {
        ...
        sceneObjects: [
          {
            ...(await import("/src/splines.js")).makePath(
              "test-water",
              "water",
            ),
            width: 6,
...
test("water controls stay live without erosion and fixed scene controls are absent", async ({
  page,
}) => {
  const errors = [];
  ...
  await expect(
    page.locator('#canyon-controls,[data-preset="0"],[data-preset="2"]'),
  ).toHaveCount(0);
