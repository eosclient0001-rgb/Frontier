...
test("GPU particles exchange actual material with a balanced ledger and no per-step CPU transfer", async ({
  ...
  await page.goto("/");
  // This is an exchange/transfer test, not a full-resolution raymarch benchmark.
  // A compact render target keeps software GPU time available for actual flight.
  await page.addStyleTag({content:"#scene{width:160px!important;height:120px!important}"});
  ...
  expect(audit.active).toBeGreaterThan(10);
  expect(audit.eroded).toBe(0); // Rain is still airborne on its birth tick.
  expect(audit.carried).toBe(0);
  ...
  await page.waitForFunction(() => frontier.iterations >= 65, null, {
    timeout: 120000,
  });
  await page.locator("#run").click();
