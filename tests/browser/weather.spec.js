...
async function harness(page) {
  ...
  await page.evaluate(async () => {
    ...
    const solver = new GPUErosion(gl),
      volume = generateVolume({ ...defaults, preset: 3, plotHeight: 0.1 });
    window.lab = {
      solver,
      volume,
      params: {
      ...
      async reset(mode, patch = {}) {
        ...
        if (mode === 2) {
          ...
          this.params.sceneObjects = [
            {
              ...makePath("river-fixture", "water"),
              width: 6,
              speed: 3.2,
              points: [
                [0, 0.7, -10],
...
test("river emits low, travels downstream, scours the bed and transports actual material", async ({
  ...
  // Deep, fast water can remain unsaturated: test entrainment first, then
  // deposition after reducing the guided flow instead of demanding both at once.
  const after = await page.evaluate(() => lab.step(40));
  ...
  balanced(after);
  const slowing = await page.evaluate(async () => {
    const D = await import("/src/domain.js");
    lab.params.riverSpeed = 0;
    lab.params.sceneObjects.forEach((n) => {
      if (n.type === "water") n.speed = 0;
    });
    lab.params.rainfall = 0;
    return lab.step(40);
  });
  expect(slowing.deposited).toBeGreaterThan(0);
  balanced(slowing);
  const inventory = await page.evaluate(async () => {
    const D = await import("/src/domain.js");
    const s = lab.solver,
      v = s.read(s.volume, D.ATLAS_SIZE[0], D.ATLAS_SIZE[1]),
    ...
    for (let i = 0; i < m.length; i += 4) {
      ...
      if (sum > v[i + 3] * D.CELL[0] * D.CELL[1] * D.CELL[2] + 1e-5) invalid++;
      loose += sum;
    }
    for (let z = 0; z < D.SIZE[0]; z++)
      for (let y = 0; y < D.SIZE[1]; y++)
        for (let x = 0; x < D.SIZE[0]; x++) {
          const a =
              ((Math.floor(z / 16) * D.SIZE[1] + y) * D.ATLAS_SIZE[0] +
                (z % 16) * D.SIZE[0] +
                x) *
              4,
            original = ((z * D.SIZE[1] + y) * D.SIZE[0] + x) * 4;
          const removed = lab.volume[original + 3] - v[a + 3];
          if (D.MIN[1] + (y + 0.5) * D.CELL[1] > 3)
            highRemoval += Math.abs(removed);
