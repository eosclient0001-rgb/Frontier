...
test("workspace CSS lays out a visible center canvas before the application starts, including embedded and short windows", async ({
  ...
  for (const [width, height] of [
    ...
    expect(r.outliner.x).toBeGreaterThanOrEqual(0);
    expect(r.outliner.x).toBeLessThanOrEqual(16);
    ...
    expect(r.properties.right).toBeLessThanOrEqual(width);
    expect(r.properties.right).toBeGreaterThanOrEqual(width - 16);
  }
});
