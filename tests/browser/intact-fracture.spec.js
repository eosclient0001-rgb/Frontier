...
test("intact marking leaves GPU volume/materials exact and live objects live; only selected touching cell is removed, with exact undo", async ({
  ...
  const report = await page.evaluate(async () => {
    ...
    const before = await s.readVolume(),
      materials = s.read(s.material, D.ATLAS_SIZE[0], D.ATLAS_SIZE[1]),
      pattern = {
        intact: true,
        detail: true,
        gap: 0.014,
    ...
    const marked = await s.readVolume(),
      markedMaterial = s.read(s.material, D.ATLAS_SIZE[0], D.ATLAS_SIZE[1]),
    ...
    const after = await s.readVolume(),
      afterMaterial = s.read(s.material, D.ATLAS_SIZE[0], D.ATLAS_SIZE[1]);
    let kept = 0,
      removed = 0;
    ...
    for (let z = 0; z < D.SIZE[0]; z++)
      for (let y = 0; y < D.SIZE[1]; y++)
        for (let x = 0; x < D.SIZE[0]; x++) {
          const i = (z * D.SIZE[1] + y) * D.SIZE[0] + x,
            j =
    ...
    const restored = await s.readVolume(),
      restoredMaterial = s.read(s.material, D.ATLAS_SIZE[0], D.ATLAS_SIZE[1]);
    return {
      connected,
      live,
      unchanged:
