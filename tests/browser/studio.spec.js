...
for (const legacyLink of [false, true]) {
  test(`${legacyLink ? "WebGL2 legacy URL" : "WebGL2 default"}: real volume edits and studio controls`, async ({
    ...
    await page.goto(legacyLink ? "/?webgl&preset=0" : "/?preset=0");
    ...
    await page.waitForFunction(
      () => frontier.settings.preset === 3 && frontier.iterations === 0,
      { timeout: 60_000 },
    );
    await expect(page.locator("#biome-name")).toHaveText("Land plot");
