    ...
    for (let z = 15; z < 100; z += 12)
      for (let y = 3; y < 30; y += 4)
        for (let x = 15; x < 100; x += 12) {
          const world = [
              D.MIN[0] + (x + 0.5) * D.CELL[0],
              D.MIN[1] + (y + 0.5) * D.CELL[1],
              D.MIN[2] + (z + 0.5) * D.CELL[2],
            ],
            index = ((z * D.SIZE[1] + y) * D.SIZE[0] + x) * 4;
          error = Math.max(
            error,
            Math.abs(
              cutVolume[index] -
...
test("water spline directs GPU river births and transport, reversal reverses current, hidden routes emit none, with no steady-state CPU transfer", async ({
    ...
    function stats() {
      ...
      for (let i = 0; i < 128 * 4; i += 4)
        if (pos[i + 3] >= 0) {
          ...
          inletError = Math.max(
            inletError,
            Math.abs(pos[i] - route.points[0][0]),
          );
  ...
  expect(result.forward.inletError).toBeLessThan(0.01);
  expect(result.backward.inletError).toBeLessThan(0.01);
  expect(result.forward.minY).toBeGreaterThan(4.3);
...
test("water spline footprint and terrain visibility change rendered pixels without changing the SDF", async ({
  ...
  const r = await page.evaluate(async () => {
    ...
    return {
      ...
      elevationPixels: changed(left, elevated),
      fallbackUnchanged: left.every((v, i) => v === fallback[i]),
  ...
  expect(r.elevationPixels).toBeGreaterThan(200);
  expect(r.fallbackUnchanged).toBe(true);
