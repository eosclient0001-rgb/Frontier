...
test("actual GPU plane cuts separate components; selected-piece deletion and one-edit undo preserve the remaining SDF", async ({
  ...
  const result = await page.evaluate(async () => {
    ...
    const s = new GPUErosion(
      ...
      a = new Float32Array(D.SIZE[0] * D.SIZE[1] * D.SIZE[0] * 4);
      ...
      for (let y = 0; y < D.SIZE[1]; y++)
        for (let x = 0; x < D.SIZE[0]; x++) {
          const p = [x, y, z].map((v, k) => MIN[k] + (v + 0.5) * CELL[k]),
            ...
            i = ((z * D.SIZE[1] + y) * D.SIZE[0] + x) * 4;
          a[i] = d;
          a[i + 3] = Math.max(0, Math.min(1, 0.5 - d / (2 * D.BAND)));
        }
    s.upload(a);
...
test("GPU Voronoi subtraction makes multiple disconnected cell fragments, leaves unpainted rock unchanged and does not read back during the cut", async ({
  ...
  const report = await page.evaluate(async () => {
    ...
    const gl = document.createElement("canvas").getContext("webgl2"),
      s = new GPUErosion(gl),
      N = D.SIZE[0] * D.SIZE[1] * D.SIZE[0],
      a = new Float32Array(N * 4);
    const world = (i) => {
      const x = i % D.SIZE[0],
        y = Math.floor(i / D.SIZE[0]) % D.SIZE[1],
        z = Math.floor(i / (D.SIZE[0] * D.SIZE[1]));
    ...
    for (let i = 0; i < N; i++) {
      const p = world(i),
        ...
        d =
          ...
          Math.min(0, Math.max(...q));
      a[i * 4] = d;
      a[i * 4 + 3] = Math.max(0, Math.min(1, 0.5 - d / (2 * D.BAND)));
    }
    s.upload(a);
    ...
    for (let i = 0; i < N; i++) {
      ...
      while (head < tail) {
        const j = queue[head++],
          x = j % D.SIZE[0],
          y = Math.floor(j / D.SIZE[0]) % D.SIZE[1],
          z = Math.floor(j / (D.SIZE[0] * D.SIZE[1])),
          p = world(j);
          ...
          for (let dy = -1; dy <= 1; dy++)
            for (let dx = -1; dx <= 1; dx++) {
              ...
              if (
                ...
                continue;
              const k = (zz * D.SIZE[1] + yy) * D.SIZE[0] + xx;
