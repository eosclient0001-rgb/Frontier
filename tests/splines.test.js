...
test("flow routes pack continuous arc lengths, width/speed and distinct path inlets; hidden means no custom flow", () => {
  ...
  assert.equal(packFlowPaths({ preset: 0 })[1], 1);
  assert.ok(packed.every(Number.isFinite));
});

test("every preset requires an explicit route and packed elevations retain per-point height plus shared offset", () => {
  for (const preset of [0, 1, 2, 3, 4]) {
    const empty = packFlowPaths({ preset });
    assert.equal(empty[0], 0);
    assert.equal(empty[1], 1);
  }
  const route = {
    ...makePath("graded", "water"),
    points: [
      [-5, 6, 0],
      [5, 2, 0],
    ],
  };
  const p = packFlowPaths({ sceneObjects: [route], waterOffset: 0.4 });
  assert.ok(Math.abs(p[193 * 4] - 6.4) < 1e-6);
  assert.ok(Math.abs(p[(193 + p[0] - 1) * 4 + 1] - 2.4) < 1e-6);
  assert.equal(p[15], 6); // captured inlet elevation, not a shared horizontal water plane
  assert.equal(p.length, 320 * 4);
});
